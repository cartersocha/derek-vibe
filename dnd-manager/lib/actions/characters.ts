...
export async function createCharacter(formData: FormData): Promise<void> {
  ...
  const { error } = await supabase.from("characters").insert(result.data);
...
export async function updateCharacter(
  ...
  const data = {
    ...
    dexterity: formData.get("dexterity")
      ? parseInt(formData.get("dexterity") as string)
      : null,
    constitution: formData.get("constitution")
      ? parseInt(formData.get("constitution") as string)
      : null,
    intelligence: formData.get("intelligence")
      ? parseInt(formData.get("intelligence") as string)
      : null,
    wisdom: formData.get("wisdom")
      ? parseInt(formData.get("wisdom") as string)
      : null,
    charisma: formData.get("charisma")
      ? parseInt(formData.get("charisma") as string)
      : null,
  };
